...
const navItemsInfo = [
  {
    ...
    expanded: [
      ...
      {
        name: "United we Bargain, Divided we Beg",
        link: "/photography/united-we-bargain-divided-we-beg",
      },
...
const navItemsHTML = () => {
  ...
  for (const navItem of navItemsInfo) {
    const isActive = window.location.href.includes(navItem.link)
      ? "active"
      : "";
    let expanded = ``;
...
const renderNav = () => {
  const div =
    window.innerWidth > 950 ? bigNav : dropDownOn ? dropDownNav : smallNav;
  if (dropDownOn) {
